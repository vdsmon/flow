"""Real-git tests for scrutinize_seat: the module is git plumbing, so fakes would prove nothing."""
...
def repo(tmp_path: Path) -> SimpleNamespace:
    ...
    marker = seed / scrutinize_seat.SELF_TARGET_MARKER
...
def _bench(repo: SimpleNamespace) -> Path:
    return repo.workspace / scrutinize_seat.BENCH_RELPATH
...
def test_creates_bench_detached_at_default(repo: SimpleNamespace) -> None:
    code, posture = scrutinize_seat.seat(repo.workspace)
    assert code == 0
...
def test_present_bench_is_never_mutated(repo: SimpleNamespace) -> None:
    scrutinize_seat.seat(repo.workspace)
    bench = _bench(repo)
    _git(["switch", "-c", "bench/inflight"], bench)
    (bench / "wip.txt").write_text("in flight\n")
    code, posture = scrutinize_seat.seat(repo.workspace)
    assert code == 0
    assert posture["bench"]["action"] == "present"
    assert posture["bench"]["branch"] == "bench/inflight"
    assert posture["bench"]["clean"] is False
    assert (bench / "wip.txt").read_text() == "in flight\n"
    assert _git(["symbolic-ref", "--short", "HEAD"], bench) == "bench/inflight"
...
def test_dry_run_writes_nothing(repo: SimpleNamespace) -> None:
    ...
    code, posture = scrutinize_seat.seat(repo.workspace, dry_run=True)
    assert code == 0
...
def test_deleted_bench_with_stale_registration_fails_predictably(repo: SimpleNamespace) -> None:
    ...
    for dry_run in (True, False):
        code, posture = scrutinize_seat.seat(repo.workspace, dry_run=dry_run)
        assert code == scrutinize_seat.EXIT_ERROR
...
def test_invoked_from_bench_targets_primary_checkout(repo: SimpleNamespace) -> None:
    scrutinize_seat.seat(repo.workspace)
    code, posture = scrutinize_seat.seat(_bench(repo))
    assert code == 0
...
def test_cli_prints_posture_json(repo: SimpleNamespace, capsys: pytest.CaptureFixture) -> None:
    code = scrutinize_seat.cli_main(["--workspace-root", str(repo.workspace)])
    assert code == 0
...
def test_non_repo_workspace_is_a_probe_error(tmp_path: Path) -> None:
    ...
    plain.mkdir()
    code = scrutinize_seat.cli_main(["--workspace-root", str(plain)])
    assert code == scrutinize_seat.EXIT_ERROR
...
def test_bench_seating_reads_the_primary_checkouts_declared_base(repo: SimpleNamespace) -> None:
    ...
    # checkout would seat a bench-invoked seat on the remote default with nothing in the posture
    # saying so.
    scrutinize_seat.seat(repo.workspace)
    (repo.workspace / ".flow").mkdir()
    (repo.workspace / ".flow" / "workspace.toml").write_text('[create_pr]\nbase = "dev"\n')
    ...
    code, posture = scrutinize_seat.seat(bench)
    assert code == 0
    assert Path(posture["target_root"]) == repo.workspace.resolve()
    assert posture["default_branch"] == "origin/main"
    assert posture["integration_branch"] == "origin/dev"
...
def test_local_runs_cover_base_revision_failure_stale_corruption_and_completed_omission(
    ...
    real_classify = scrutinize_seat.lease.classify
    ...
    monkeypatch.setattr(scrutinize_seat.lease, "classify", classify)
    code, posture = scrutinize_seat.seat(repo.workspace)
    assert code == 0
...
def test_unresolvable_configured_base_falls_back_and_says_so(repo: SimpleNamespace) -> None:
    ...
    # one: accepted silently, it can never equal a branch name, so the seat's
...
def test_refuses_outside_the_self_target(repo: SimpleNamespace) -> None:
    ...
    with pytest.raises(scrutinize_seat.SeatError, match="self-target"):
        scrutinize_seat.seat(repo.workspace)
