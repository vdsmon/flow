"""Cross-worktree memory dedup: main-checkout and worktree runs converge on one store.

A ticket run individually (main checkout) and the same ticket run as one of N concurrent
pool worktrees must hit the SAME knowledge store with the SAME idempotency id
