...
| Script | Role | Surface / touches |
|--------|------|-------------------|
| `metric.py` | Metrics calculator: shipped tickets/week, time-to-PR, friction events/run, revert-rate, and arm-compare (per-arm flow-vs-control comparison) — from ship-event and friction-jsonl evidence (revert-rate joins ship-events to `bd history`). | (via `recall.py --metric`) |
