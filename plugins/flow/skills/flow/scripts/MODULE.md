...
| `forge_bitbucket.py` (lib) | Bitbucket adapter over the `brinta-ai bitbucket` proxy (absorbs ship-it): detect/open PR, CI rollup from the PR commit-status endpoint, commit-pinned `src/<sha>/<path>#lines-x:y` source URLs, CodeRabbit review-thread fetch + verified resolve (`.resolution != null`), `set_default_reviewers` (GET `2.0/user` author + GET `default-reviewers`, drop author by `account_id`, PUT `{reviewers:[{uuid}]}`). One credential store (`brinta-ai setup`), `2.0/` prefix stripped for the proxy root, bodyless `(HTTP …)` successes mapped to None. | — |
