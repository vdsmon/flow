...
| `seam_check.py` | Structurally validate documented absolute, call-local-harness facade commands against flowctl's allowlist and argparse surfaces. Reject cwd-dependent facades, host-specific public recipes, and stale direct scripts. Enforce descriptor, role, registry, and module-map contracts. Flag a recipe span that binds a zsh-reserved or otherwise unsafe shell name (`zsh_unsafe_binding_problems`). | `[--verbose]`. Exit 1 on drift |
