...
| Script | Role | Surface / touches |
|--------|------|-------------------|
| `metric.py` | Metrics calculator: shipped tickets/week, time-to-PR, friction events/run, revert-rate, and arm-compare (per-arm flow-vs-control comparison) — from ship-event and friction-jsonl evidence (revert-rate joins ship-events to `bd history` AND scans the default-branch git log for reverts, emitting durable revert events via observe_ship_event). | (via `recall.py --metric`) |
