...
| `evolve_self_merge.py` | Self-merge gate (the `merge` stage core): pure `decide(labels, is_maintainer, auto_merge_hot, ci_status, planned_files) → {action, is_hot, reason}`, where `is_hot` is the `hot` label OR a guard-file hit in `planned_files` (reuses `triage.is_hot_change`). The stage acts on it: a hot bead gets an independent reviewer subagent (§6A) before `forge_cli merge`. | `--workspace-root --key --ci-status`; consumed by `references/stage-merge.md` |
...
| `version.py` | Version-derivation + merge-time stamp seam (epic flow-6gx): compute the next plugin patch version (read the current `plugin.json` version on a git ref, bump PATCH +1, preserve MAJOR.MINOR) and surgically write it into both version files (`plugin.json` top-level + the `marketplace.json` flow entry), preserving JSON formatting. The per-PR bump is gone; `stamp` runs at merge time. | `next [--ref <ref>] [--cwd <path>]` prints JSON `{"ref","current","next"}`; `stamp [--ref <ref>] [--cwd <path>]` writes both files then prints the same JSON; exit 0 = ok, 2 = tool error. `stamp` consumed by `references/stage-merge.md` (§3). |
...
| `harness_corpus.py` (lib) | Frozen decider-fixture corpus loader/validator + replayer (regression-eval, epic flow-63q): replays held_in/held_out cases from the sibling `harness_corpus.json` data file against the four pure deciders (`evolve_select.partition`, `evolve_drain.decide`, `evolve_self_merge.decide`, `triage.is_hot_change`); the injectable `resolve=` is the candidate-checkout seam. | Frozen by `tests/test_harness_corpus.py` (full replay each CI run); imported by harness_eval |
| `harness_eval.py` | Regression-eval scorer (epic flow-63q): replays the frozen corpus against a candidate skill-checkout AND a baseline via an isolated subprocess driver per checkout; emits per-split pass/regress delta + `non_regression`; raw data only, gating policy lives in the merge gate (flow-63q.4). | `score --candidate <scripts-dir> [--baseline <scripts-dir> --corpus <file> --timeout-secs N]` prints delta JSON; exit 0 = clean, 1 = bad dirs, 2 = corpus/driver error, 3 = regression. `drive` is the internal stdin-JSON replay driver. |
