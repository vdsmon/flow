...
| `_evolve_common.py` (lib) | Shared evolve-drain helpers: `ToolError`/`NotMaintainer`, tool-call ok-wrapper, tolerant JSON-list parse, `feature/<key>` branch→key regex, bead label sets, worktree-pool run-dir resolution. | imported by evolve_reap, evolve_select, evolve_drain, evolve_session_cleanup |
