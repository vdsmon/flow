...
| `version.py` | Version-derivation seam: compute the next plugin patch version (read the current `plugin.json` version on a git ref, bump PATCH +1, preserve MAJOR.MINOR). Keystone of epic flow-6gx, shipped ahead of its callers (flow-6gx.2 drops the version files off the per-PR content path, flow-6gx.4 stamps the derived version at merge time) — caller-less by design until then. | `next [--ref <ref>] [--cwd <path>]` prints JSON `{"ref","current","next"}`; exit 0 = ok, 2 = tool error. No callers yet (keystone). |
