...
# Namespace-drift surfaces (flow-glrn): prose and manifest strings that enumerate the public
# namespaces are regenerated by nothing, so a retired namespace survives in them silently (the
# witnessed case: `maintain` outlived its own retirement in marketplace.json and command-target.md,
# invisible to every gate). The comma list tolerates an optional Oxford `and`; a token is the drift
# when it is not a live static namespace.
_NAMESPACE_LIST_RE = re.compile(r"\bthe ((?:[a-z]+,\s+)+(?:and\s+)?[a-z]+) namespaces?\b")
_STATIC_ROOTS_RE = re.compile(
    r"((?:`[a-z]+`,\s+)+and\s+`[a-z]+`)\s+are always parsed\s+as static roots first"
)
_NAMESPACE_TOKEN_RE = re.compile(r"[a-z]+")
...
def _namespace_surfaces(skill_root: Path) -> tuple[tuple[Path, str], ...]:
    """(path, label) per hand-authored surface that enumerates namespaces.

    The manifests sit outside the skill tree: plugin.json one level above it, marketplace.json at
    the repo root. Resolved per call so tests can point skill_root at a fixture tree; a fixture too
    shallow for the manifest ancestors contributes no manifest surface.
    """
    surfaces = [
        (skill_root / "references" / "command-target.md", "command-target.md static roots"),
    ]
    parents = skill_root.parents
    if len(parents) > 1:
        surfaces.append((parents[1] / ".claude-plugin" / "plugin.json", "plugin.json"))
    if len(parents) > 3:
        surfaces.append((parents[3] / ".claude-plugin" / "marketplace.json", "marketplace.json"))
    return tuple(surfaces)


def _namespace_drift_problems(skill_root: Path, registry: Registry) -> list[str]:
    """Every namespace token a hand-authored surface names that the registry lacks."""
    allowed = set(registry.static_namespaces)
    problems: list[str] = []
    for path, label in _namespace_surfaces(skill_root):
        try:
            text = path.read_text(encoding="utf-8")
        except OSError:
            continue
        flat = " ".join(text.split())
        lists = [m.group(1) for m in _NAMESPACE_LIST_RE.finditer(flat)]
        lists += [m.group(1) for m in _STATIC_ROOTS_RE.finditer(flat)]
        for listed in lists:
            for token in _NAMESPACE_TOKEN_RE.findall(listed):
                if token == "and" or token in allowed:
                    continue
                problems.append(f"{label} names a namespace the registry does not have: {token}")
    return problems
...
def check(
    ...
    *,
    ...
    problems.extend(_namespace_drift_problems(skill_root, registry))
