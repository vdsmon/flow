...
| `record-baseline` | 0=ok, 2=git-error | Writes `<ticket-dir>/baseline.json` with `{stage, head_sha, origin_sha, planned_files, blobs}`. `head_sha` is the diff anchor and is recomputed from live HEAD on every record. `origin_sha` is the ownership anchor: written on the first record, preserved by every later one, so the post-implement reconcile widens the owned set without moving what `check-ownership` scans from. Records what it is told, including an empty `planned_files`, because the refusal belongs in the captures that interpret it. |
