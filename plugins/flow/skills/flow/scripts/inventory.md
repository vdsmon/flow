...
| `record-baseline` | 0=ok, 2=git-error | Writes `<ticket-dir>/baseline.json` with `{stage, head_sha, origin_sha, planned_files, blobs}`. `head_sha` is the moving implement-diff anchor and is recomputed from live HEAD on every record. `origin_sha` is the stable review and ownership anchor: written on the first record and preserved by every later one, so the post-implement reconcile widens the owned set without hiding committed work. Records what it is told, including an empty `planned_files`, because the refusal belongs in the captures that interpret it. |
| `capture-implement-diff` | 0=ok, 1=missing-baseline / gitignored planned file / empty `planned_files`, 2=git-error | Writes `<ticket-dir>/implement.diff` via `git diff --binary --raw`. `--binary` is what makes the patch appliable by stage-commit step 5's `git apply --cached --binary`. An empty `planned_files` exits 1 rather than capturing: with no pathspec `git diff` covers the whole repository, and an empty owned set can never legitimately produce a scoped patch. |
| `capture-review-diff` | 0=ok, 1=missing-baseline / invalid present `origin_sha` / gitignored planned file / empty `planned_files`, 2=git-error | Writes `<ticket-dir>/review.diff` from stable `origin_sha` via `git diff` with no `--binary`/`--raw`, so committed planned work remains visible after baseline re-records and binary content is elided to a `Binary files ... differ` line instead of inlined. A legacy baseline with no `origin_sha` falls back to `head_sha`; an invalid present value is refused. It keeps the same `planned_files` scope and index restore as `capture-implement-diff`; a separate function rather than a shared helper means the review path cannot alter the guard path. The empty-`planned_files` refusal is written out in both functions for that same reason, and nothing downstream of this payload checks ownership, so a repo-wide review diff would reach a reviewer unfiltered. Read by code_review's reviewer, never applied. |
| `check-ownership` | 0=ok, 3=ownership violation (unowned paths), 1=missing/malformed baseline, 2=git-error | `{ok, planned_files, changed, unowned_changes, ownership_anchor, anchor_source, committed_scan_empty}` JSON. Branch-wide: scans the dirty working tree AND the committed delta `baseline.origin_sha..HEAD`, so a rogue mid-implement commit is seen too, and it stays seen after the reconcile re-records the baseline. `anchor_source` is `origin_sha`, or `head_sha_fallback` for a baseline written before `origin_sha` existed (the pre-change behaviour, reported rather than silent). `committed_scan_empty` says the committed half listed no path between the anchor and HEAD; it describes THAT HALF ALONE and never means the gate checked nothing, since the ordinary healthy run has every edit uncommitted and is still fully checked by the working-tree half. What signals a scan that covered nothing is the conjunction of `ok`, an empty `changed`, and `committed_scan_empty`. Wired as stage-commit step 2b. Filename-level by design; hunk-level ownership stays a deliberate non-goal (bd flow-bq4). THIS is the content-ownership commit gate AGENTS.md names. |
