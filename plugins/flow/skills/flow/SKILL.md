...
**`ExitPlanMode` with the plan = Gate 1, the only human gate.** Gate on an INDEPENDENT confidence rating (the `advisor` tool — it auto-forwards the transcript; or a `general-purpose` `Agent` if advisor is absent — on Fable models it always is, by design: skip the probe), never self-scored. **< 90% → do NOT `ExitPlanMode` yet:** first exhaust every reachable read-only artefact (Read/Grep/Glob, an `Explore` agent, WebSearch/WebFetch, read-only MCPs), then for a gap that needs user action ask via `AskUserQuestion` with specifics, then re-assess. Library-API claims must be Context7-verified. The rating is part of the plan, surfaced unprompted, every time — present only at >=90%, or when every reachable source is exhausted and the residual is documented as a risk. If the workspace opts into e2e, the plan also settles the **e2e recipe** here (while live auth is present). The plan also **proposes a verification lane** (`express`/`light`/`full`, conservative — `express` only for behavior-preserving, tightly-bounded work); you approve it with the plan or override, or pre-set `--lane`. An `express`/`light` lane **skips the confidence probe** — your gate approval is the vetting that the producer audit plays on the drain side — and lightens the tail (TDD relax for `express`, reflect collapse); a hot change (guard-file touch) is always `full`. Detail: `references/verb-spec.md`.

On approval (normal mode): the bootstrap (`flow_worktree.py create`) persists the plan, seeds the worktree, and stamps `planned_files` + `commit_type` + `commit_summary` (+ `e2e_recipe`, + the approved `--lane` when not `full`) into frontmatter so the tail never pauses to ask — then `EnterWorktree(path="<worktree>")` switches this session in and you **continue straight into the `do` loop below** (its `init` resumes the spec-seeded run at `implement`).
...
4. **Orchestration loop** — repeat until done:
   ...
   d. Dispatch by `handler_type`:
      ...
      - **`subagent:<type>`** — If `descriptor.reference_doc` is present, Read `${CLAUDE_SKILL_DIR}/${descriptor.reference_doc}` first (e.g. `references/stage-plan.md`, `references/stage-implement.md`) — it carries the per-stage protocol. Then spawn an Agent embedding that protocol:
        ```
        Agent(
