...
1. **Author the body** per the template above. Gather inputs:
   ...
   - the premeditated file set + per-file rationale: `$TICKET_DIR/stages/plan.out` — its `## Files to change` bullets (a backticked path + a one-line note each) seed the `## Changes` bullets below (see the authoring note after this list); plus the ticket (`ticket.json`) for the overall why,
