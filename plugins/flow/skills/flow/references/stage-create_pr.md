...
Opens a PR for the run's feature branch — ready for review by default, or a draft when `[create_pr] draft = true` in `workspace.toml` (`create_pr.py` reads it; `--draft` overrides). Inline handler for GitHub workspaces (flow's own dogfood wires `create_pr = "inline"`); the bare plugin default stays `none`, so non-GitHub workspaces never reach here.
...
1. Open or resolve the PR:
   ...
   - Exit 2 → git/gh error; surface stderr, set `STATUS=failed`.
