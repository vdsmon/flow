...
4. **Open or resolve the PR:**
   ...
   ```
   The base branch resolves from `[create_pr] base` in `workspace.toml`, default `main`; an explicit `--base` overrides both. When the run frontmatter carries `hotfix = true`, add `--hotfix` and never `--base` (the script refuses the pair): the PR opens ready for review against the remote default branch, ignoring the `[create_pr]` base and draft settings, because a hotfix always targets what production builds from. A workspace convention that ordinary PRs target an integration branch (a `base = "dev"` setting, a "PRs always target dev" rule in the repo's agent docs) does NOT apply to a hotfix; a hotfix PR open against the remote default is correct by design, never a defect to fix, and the driver never retargets it or moves its branch onto the integration branch afterwards (witnessed 2026-08-14: a driver "corrected" a hotfix PR onto dev and the fix had to be re-cut off the default branch).
