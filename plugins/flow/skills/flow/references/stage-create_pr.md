...
- Size the body to the change. A small change (about three files or fewer, no behavior surprise) gets the minimum: summary line, a 1-3 sentence lead, `## Changes` bullets of one short clause each, `## How to verify`, and nothing else. The optional sections exist for changes that earn them; reaching for every section on a routine fix is the failure mode, not thoroughness.
...
2. **Humanize (mandatory-when-present).** If `humanize:humanize` is in your available skills you MUST run the authored body through it. Two things the skill's behavior forces (verified against a filled template):
   ...
   **Do not end the turn on the rewrite — continue the stage.** `create_pr` is an INLINE handler running in the driver conversation, so the humanize Skill executes there and its Final rewrite would normally end the turn. Do NOT stop there: take the Final rewrite and CONTINUE in the SAME reply — emit step 3's `pr_body.md` heredoc, run step 4's `create_pr.py`, capture step 5's `.out`, and issue the do-loop `advance`. Otherwise the do-loop stalls until the human continues it (witnessed twice: flow-gfz5, flow-qdal; friction `8f22583e41ee443fb6eb104b32bceece`). This is the primary instance of the general inline-skill turn-continuation rule in `references/delivery-loop.md`.
