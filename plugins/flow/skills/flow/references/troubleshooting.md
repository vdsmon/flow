...
- **A shim invocation creates or drops an untracked file (e.g. `uv.lock`) inside a capsule's authoritative source root.** mise-backed shims (`mise exec`, a bare `python3`/`mise` resolving through the shim) can write project files as a side effect of resolving a tool. A cognitive capsule's read-only postcondition treats any such untracked-content change as authoritative-repository mutation and refuses with `read_only_violation`, naming the exact path (see the containment note in `references/delivery-loop.md`). The remedy is prevention: never run a bare `python3`/`mise`/shim-backed command against the authoritative worktree between a capsule launch and its post-receipt.
...
- **A Claude Code host's own background Bash supervisor gets killed mid-invocation, wedging a cognitive-worker journal at `running`.** Background Bash tasks in this host are not guaranteed to survive the session that spawned them (witnessed twice supervising long assessor/reviewer launches; a Codex host's own process supervision survived the same session). The remedy is a host-owned detached supervisor, not an engine change: launch the long-running cognitive-worker invocation from a Bash call using `start_new_session` semantics so the child process group survives the spawning shell, then poll the invocation's journal/result files (mtime, `state` field) from a separate, cheap foreground call instead of blocking on the original Bash call's own lifetime. flow's engine deliberately does not self-daemonize: `run_provider_process` owns the provider's stdout/stderr pipes directly so `ProcessEvidence.stdout_eof`/`stderr_eof` stay meaningful proof of a clean exit, and it must not detach from the invoking process. If a supervisor is lost anyway, `cognitive-worker recover-invocation --logical-invocation-id <id> --artifact-root <root> --capsule-root <root> --reason <reason>` is the recovery path (it calls `CognitiveWorkers.cancel`, which only quarantines a `running` invocation once boot/process-group evidence proves the prior launch is dead), not a home-grown relaunch.
