...
- `--unattended` asks the driver not to prompt during an already approved,
  bootstrapped delivery. It conflicts with `--verify` and `--hotfix`. A fresh target always stops
  at the human plan gate without mutation; unattended mode cannot authorize planning.
- `--hotfix` enters the production-incident lane for a fresh attended target: the same lifecycle, expedited. The plan collapses to a short root-cause-and-fix confirmation at the same gate (the human is present mid-incident), the run branch is minted `hotfix/<ticket>-<slug>` off `@default` so it cuts from the freshly-fetched remote default, the dispatcher runs subagent-wired stages inline in the driver (`none` stays skipped; a lane still never re-enables a stage), and the PR opens ready for review against the remote default branch. Branch base and PR target are lane constants, not workspace settings: the workspace's ordinary-PR base (`[create_pr] base`, a repo rule that PRs target an integration branch) never applies to a hotfix, both scripts refuse a substituted base, and the driver never rebases the branch onto the integration branch or retargets the PR by hand. On green CI with a clean review the driver merges it through the forge seam (`references/stage-review_loop.md` §Hotfix merge); delivery is always that PR, never a direct push to the default branch. It conflicts with `--unattended`: a hotfix is attended by definition. Verification-lane rules are unchanged: the driver proposes `express`, and section 2's hot and high-stakes clamps to `full` still apply, so a hotfix that touches guard files keeps the guard-property review.
