...
4. Re-pin the session on the run root before the first command that writes there. On Claude Code that is `EnterWorktree` with the absolute `run_root` path, and only when the session is not already rooted there (the switch refuses "is the current working directory" for a session that `worktree create` left inside the run root, one wasted call in FT-1681); hosts without a native switch skip this step. Do it proactively, at adoption, rather than waiting for the refusal: the host blocks a confined write with "This session is isolated in the worktree" when the session is rooted elsewhere, and every blocked call is a wasted round trip at a stage boundary (flow-l6lk, 18 of 71 tool errors in the 2026-08-10..13 delivery window). The pin is per attempt, not durable, so §Handler downgrade's rule still stands for a refusal that arrives later in the run.

   Re-pinning does not lift the second refusal shape, "too complex to verify that it stays inside the worktree; break it into plain, separate commands": an isolated session's host refuses any Bash it cannot statically prove stays inside the run root, and that classifier reads `;`, `&&`, `||`, pipes, shell variables, redirects into a path, `git -C` at another checkout, and unexpanded globs as too complex, however plain the facade call inside them is. FT-1687 paid it ten times on an engine that already re-pinned at adoption, eight of them on ordinary dispatch and tracker calls wrapped in `> file; echo rc=$?; head`. Inside an isolated session, issue every facade call as one plain command with absolute paths and nothing chained around it, read its output as the tool result, and put anything multi-step into a script file under the run root and run that file; a subagent working in the run root follows the same rule, since it cannot re-pin at all.
