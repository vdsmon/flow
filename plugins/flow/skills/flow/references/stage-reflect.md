...
1. Bundle the reflect inputs:
   ...
   - Exit 0 → JSON payload to stdout: `{ticket, run_id, state,
     ...
     friction_recurrence[], recalled_entries[], reflect_config}`.
...
2b. **Machinery reflection (lens B — gated; mandatory when ON and the run hit any friction).** SKIP this entire step unless `reflect_config.machinery` is true. It is false by default: a delivery workspace is not flow's source tree, so there is nothing there for a machinery edit to fix and flow-internal findings are noise. When the flag is off, do not record `MACHINERY:` entries and do not apply harness fixes; go straight to step 2c. When it is on (flow's own repo), run it in full. The steps above point the lens DOWN at the ticket's domain (the code, the tax rules, the library). This step points it UP at the harness that produced the work: did `FLOW`'s own scripts, stages, exit codes, handler dispatch, and orchestration loop serve the run, or fight it? This is the feedstock an installed skill-polishing capability consumes — produce it whether or not a human asked, at the depth of an engineering review, not a vibe check.
   ...
   **Check the branch before authoring a payload.** `machinery_edit` refuses (exit 2) when
   ...
   - **PROPOSE + RECORD, do not self-apply unattended.** Structural changes (the orchestration driver, the dispatch loop, a script rewrite), anything touching a file the fleet is actively mid-stage on, or anything you are not high-confidence is strictly correct. The blast radius across concurrent runs is too large to self-apply. Here the `MACHINERY:` entry + the human note ARE the deliverable, and recording them IS acting on the finding: it is durable, it survives the run, it is findable later. That is the requirement. If a skill-polishing or refactor capability happens to be installed, you MAY invoke it here to carry the recorded finding into a reviewed edit while the context is fresh, but that is a loose best-effort dependency, not a mandate, and the run does not block on it: the record stands on its own when no such skill exists. A human gate sits on anything that would be pushed.
     - **Record for the foreman; reflect files no beads.** A `MACHINERY:` entry in `knowledge.jsonl` IS the finding's durable record, and the record is where reflect stops: beads are minted in exactly one place, the foreman's sweep (`foreman.md` §Friction handling), which reads these entries and the friction log across every workspace and files only what carries a delivery-workspace witness or a second real-cost witness. A self-target run's introspection never becomes backlog on its own; that is the no-flow-examining-flow-beads rule, and it also leaves the dedup net with a single producer. Make the entry carry what the foreman needs to mint later: the file:line evidence, a candidate dedup anchor (`<primary-relfile>::<short-symptom>`, keep the `::` separator), and, when the finding touches a hot file, such as `SKILL.md` / `stage-registry.toml` / `CLAUDE.md` / `AGENTS.md` / a wired handler, or a safety-machinery guard file
       <!-- flow:guard-files:begin -->(`_atomicio.py`, `_locking.py`, `diff_extract.py`, `dispatch_stage.py`, `flow_friction.py`, `flow_launcher.py`, `flow_worktree.py`, `flowctl.py`, `lease.py`, `machinery_edit.py`, `snapshot.py`, `state.py`),<!-- flow:guard-files:end -->
       say HOT in the entry so any bead the foreman mints rides the high-scrutiny lane and its merge-time guard-property review.
   ...
   **Deterministic closing sub-step: recurrence escalation.** MANDATORY, best-effort, no judgment involved — run the fixed command:
   ...
   ```
   Propose-only: it consumes the same `friction_recurrence` block already in the bundle (step 1) and files a `recurrent`-labelled bead per friction class that recurred `>=K` times since its LATEST claimed fix (not the aggregate since-earliest count), where `K` and the exempt-anchor set are `[reflect]` workspace.toml knobs (`recurrence_escalation_k` default 3, `recurrence_exempt_anchors` default `[planned_files]`). It never gates, reopens, or launches anything — the bead carries no `machinery` label, so nothing routes it without a triage; it exists to surface a fix that did not hold to the human. Idempotent: dedup is keyed on the bare anchor, so re-running it every reflect files at most one bead per anchor ever. Best-effort: a non-zero exit logs one line and reflect continues, it never fails the stage. Auto-dormant without a route back to the flow repo (no `[maintainer]` marker anywhere on the route) — nothing is filed, the normal outcome in such a workspace, not an error.
...
2c. **Project knowledge reflection (lens C — gated, default ON).** SKIP this step only when `reflect_config.claude_memory` is false. It points OUTWARD at the project's own durable knowledge, the stores that compound across EVERY session in this project, not just Flow runs (Flow's `knowledge.jsonl` is namespaced and recalled during delivery planning; these are broader). Two targets, deliberately different blast radii:
   ...
   - **Project `CLAUDE.md` / `AGENTS.md` (PROPOSE, do NOT apply).** If the run taught a durable PROJECT RULE worth codifying in the repo's checked-in `AGENTS.md` (a convention, a recurring gotcha, a workflow rule every later session in this repo should follow), SURFACE it as a one-block proposal in the reflect output: `Proposed AGENTS.md addition:` followed by the exact text and the one-line reason. Do NOT edit `AGENTS.md`. It is a repo artifact, and editing it at reflect-time forces a commit that re-triggers the CI + review loop (the same post-PR-churn boundary step 2 enforces). The human applies the proposal on their own schedule. Emit nothing here if the run taught no repo-rule-worthy fact.
...
3b. **Supersede recalled entries this run disproved (lens A — always on).** For each `recalled_entries` item from the bundle, judge whether THIS run's `final_diff` disproves the behavior the entry asserts. The recalled entry was live context for this run; if your own change made it false, the dead entry must be retired so the next recall does not surface stale truth.
   ...
   - **Anything ambiguous, indirect, or inference-based** — the entry merely looks stale, or the contradiction is NOT in `final_diff` — do NOT auto-supersede. Surface a one-line proposal note in the human-facing reflect output instead (`Proposed supersede: <id> — <why it may be stale>`), a binding skeptic correction the human adjudicates. The auto path is reserved for diff-grounded disproof.
...
5. Check the ship state:
   ...
   - `state == "not_shipped"` or `"indeterminate"` → not landed (or no confirming evidence yet).
     ...
     For a run whose PR merges AFTER reflect this is the EXPECTED post-PR#277 outcome (the bead is still open, or the merge has not landed): the post-merge reap seam (the `worktree_janitor` sweep or `FLOW ticket finalize`, via `observe_at_close.py`) then owns the observation, reading `not_yet_observed` and freezing the event before it tears the worktree down. Reflect's own step-6 observation still fires when a close races ahead of it: the foreman can merge a parked green PR mid-run before reflect, so at reflect the bead is already closed+merged and `is_shipped` reads `not_yet_observed`.
