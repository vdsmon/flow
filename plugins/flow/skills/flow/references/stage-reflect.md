...
2b. **Machinery reflection (lens B — gated; mandatory when ON and the run hit any friction).** SKIP this entire step unless `reflect_config.machinery` is true. It is false by default: a delivery workspace is not flow's source tree, so there is nothing there for a machinery edit to fix and flow-internal findings are noise. When the flag is off, do not record `MACHINERY:` entries and do not apply harness fixes; go straight to step 2c. When it is on (flow's own repo), run it in full. The steps above point the lens DOWN at the ticket's domain (the code, the tax rules, the library). This step points it UP at the harness that produced the work: did `FLOW`'s own scripts, stages, exit codes, handler dispatch, and orchestration loop serve the run, or fight it? This is the feedstock an installed skill-polishing capability consumes — produce it whether or not a human asked, at the depth of an engineering review, not a vibe check.
   ...
   **Check the branch before authoring a payload.** `machinery_edit` refuses (exit 2) when
   ...
   - **PROPOSE + RECORD, do not self-apply unattended.** Structural changes (the orchestration driver, the dispatch loop, a script rewrite), anything touching a file the fleet is actively mid-stage on, or anything you are not high-confidence is strictly correct. The blast radius across concurrent runs is too large to self-apply. Here the `MACHINERY:` entry + the human note ARE the deliverable, and recording them IS acting on the finding: it is durable, it survives the run, it is findable later. That is the requirement. If a skill-polishing or refactor capability happens to be installed, you MAY invoke it here to carry the recorded finding into a reviewed edit while the context is fresh, but that is a loose best-effort dependency, not a mandate, and the run does not block on it: the record stands on its own when no such skill exists. A human gate sits on anything that would be pushed.
     - **Record for the scrutiny; reflect files no beads.** A `MACHINERY:` entry in `knowledge.jsonl` IS the finding's durable record, and the record is where reflect stops: beads are minted in exactly one place, the scrutiny's sweep (`scrutinize.md` §Friction handling), which reads these entries and the friction log across every workspace and files only what carries a delivery-workspace witness or a second real-cost witness. A self-target run's introspection never becomes backlog on its own; that is the no-flow-examining-flow-beads rule, and it also leaves the dedup net with a single producer. Make the entry carry what the seat needs to mint later: the file:line evidence, a candidate dedup anchor (`<primary-relfile>::<short-symptom>`, keep the `::` separator), and, when the finding touches a hot file, such as `SKILL.md` / `stage-registry.toml` / `CLAUDE.md` / `AGENTS.md` / a wired handler, or a safety-machinery guard file
       ...
       say HOT in the entry so any bead the seat mints rides the high-scrutiny lane and its merge-time guard-property review.
...
5. Check the ship state:
   ...
   - `state == "not_shipped"` or `"indeterminate"` → not landed (or no confirming evidence yet).
     ...
     For a run whose PR merges AFTER reflect this is the EXPECTED post-PR#277 outcome (the bead is still open, or the merge has not landed): the post-merge reap seam (the `worktree_janitor` sweep or `FLOW ticket finalize`, via `observe_at_close.py`) then owns the observation, reading `not_yet_observed` and freezing the event before it tears the worktree down. Reflect's own step-6 observation still fires when a close races ahead of it: the seat can merge a parked green PR mid-run before reflect, so at reflect the bead is already closed+merged and `is_shipped` reads `not_yet_observed`.
