...
Per closed MACHINERY-fix bead (a `.flow/<namespace>/knowledge.jsonl` entry whose body starts with `MACHINERY`, grouped by `ticket`), reports whether the `(stage, type, anchor)` class it claimed to fix recurred afterward. Distinctive-anchor selection reuses `friction_recurrence`'s exact DF band + machinery ceiling-exemption, but the per-bead join is structural: `claimed_tuples` are the `(stage, type, anchor)` triples of PRE-fix friction (`ts <= fix_ts`) sharing a claimed anchor, and a later friction entry (`ts` strictly `>` `fix_ts`) recurs only when one of its own triples is in `claimed_tuples`. Verdict is `recurred` / `clean` plus evidence (`post_fix_count`, `claimed_anchors`, `claimed_tuples`, `recurrence_run_ids`, `stages`, `types`, `recurrences`, `fix_shas`). A bead is `unmeasurable` (still counted `clean`, never a third verdict) when it claims no distinctive anchor, has no usable fix timestamp, or its claimed anchors never occurred in pre-fix friction; the per-bead `unmeasurable_reason` field names which (`no-distinctive-anchor` / `no-fix-ts` / `no-pre-fix-occurrence`). Default output is a per-bead table; `--json` emits `{beads, totals, resolved_workspace_root}`, where `totals` carries `fix_beads`/`recurred`/`clean`/`unmeasurable`/`recurrence_rate` (recurred / fix_beads, over ALL fix_beads including the unmeasurable ones). `--namespace` is optional (auto-resolves from workspace.toml when omitted). This is a lifetime metric: `--since`/`--until` are accepted for CLI-surface symmetry but IGNORED.
