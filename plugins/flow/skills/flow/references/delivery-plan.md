...
```bash
FLOW_HARNESS="<harness>" "<facade>" preflight check --workspace-root . < /dev/null
```

`status: unconfigured` is silence and costs nothing. The configured command should be a check-then-login wrapper (brinta's `mise sso` verifies in about a second while the session is valid and opens the interactive login only on expiry), which is exactly why it runs here and never after the gate: the human completes the browser round-trip in seconds while present, where a stage agent discovering the same expiry mid-run can only stop and wait. A failed or timed-out check is reported to the human and never blocks planning by itself. This plan-time run is the courtesy, not the enforcement: `worktree create` runs the same preflight itself at the bootstrap chokepoint and refuses (exit 9, nothing created) when it fails, using the attended check on interactive launches and the silent probe on unattended ones (wired 2026-08-04 after two of the three runs following brinta's config skipped the documented manual step). Running it here just means the credential is warm before the approval moment instead of pausing it. Warm has a lifetime: when planning spans a break long enough for the credential to expire (an overnight gap, a session resumed the next day), run the check again just before presenting the gate, because otherwise the bootstrap chokepoint runs the interactive login inside `worktree create` and the driver sees only a Bash call that hangs. FT-1687 planned across two days on one 08-17 check, watched its 08-18 `worktree create` sit on the browser login for five minutes, aborted it, and troubleshot `aws sso` by hand until the human named the configured task twice; the check that would have opened the same login while the human was reading the plan costs one call. Invoke it as its own foreground Bash call: never chained behind another command, never piped into anything, with stdin closed as the recipe shows and an explicit timeout past the host default (the engine gives the configured command 600s, so pass 600000ms; the 120s default kills the call mid-login and pushes the browser round-trip into the background, where nobody can attend it). The wrapper's own output lines are the only thing telling the human what to click, and a pipe buffers them until the process exits, so a `| tail` hides the login URL for exactly as long as it matters. Then read the record rather than assuming it: only `status: ok` (or `unconfigured`) passed, `failed` and `timeout` exit 2, and a check whose record nobody read is not a check. On 2026-08-20 a session chained the check behind the ticket claim and piped it through `tail -10`: the output read `--- preflight ---` and then nothing for minutes, and the human diagnosed the stalled login from a browser screenshot.
