...
3. Draft the plan with these sections:
   ...
   - **Files to change** — explicit paths, each with a one-line note on what
     ...
     **Flag any binary deliverable explicitly.** A binary file in the change set (an `.xlsx` template, an image, a compiled fixture) cannot be produced by the text-only implement subagent — mark its path as binary (e.g. `path/to/x.xlsx (binary, orchestrator-copied)`) so the orchestrator copies it into the worktree post-implement rather than expecting the subagent to write it (the post-implement reconcile in `references/verb-do.md`).
