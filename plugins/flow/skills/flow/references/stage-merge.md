...
The terminal self-merge stage. Runs **after `reflect`**, so every output of the run is committed before the PR lands. Default handler is `none` (the generic pipeline and every delivery workspace skip it — the human keystone holds); flow's own self-target workspace wires `merge = "inline"`. This is the in-run half of self-merge: an evolve run that reached green + review-clean (`review_loop`) merges its own PR instead of waiting for a deferred drain pass.
...
- **`action` `"skip"`** → leave the PR as-is for the human (the normal outcome in a delivery workspace, and for a held hot ticket), `STATUS=completed`. Done. An unattended driver records this as a parked green PR in durable evidence; a later drain distinguishes it from an orphan without consulting host session state. On an eval-driven skip (`eval_status` is `regressed`/`error`), first post a PR comment naming `regressed_cases` from the verdict (mirrors §2's `held_guard` pattern) so the human sees WHICH frozen cases moved.
...
If the reviewer reports `property_removed: true` → **do NOT merge.** Post a PR comment naming the property, report it under `held_guard` in the stage output, and `STATUS=completed` (the PR stays ready for the human). Only a clean review (`property_removed: false`) proceeds to §3.
