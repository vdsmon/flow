...
Read this at plan time — `delivery-plan.md`'s recipe-settling step — the moment you settle a ticket's `e2e_recipe`.
...
- **Env-teardown** — the stop for whatever env-prep started. A recipe that brings up a container, service, or daemon names the literal command that stops it (`docker stop <name>`, `docker compose down`); a recipe whose env-prep starts nothing says `none`. Without this line every container-backed recipe leaks by construction: bring-up was specified, stop was nobody's job, and the container outlives the run (witnessed 2026-07-31, a per-worktree devcontainer still up after its e2e finished). `stage-e2e.md` step 4 runs it after evidence capture, pass or fail.
...
`FLOW_HARNESS="<harness>" "<facade>" worktree create --e2e-recipe` is what seeds the settled recipe into frontmatter; it is the only flag this doc names.
