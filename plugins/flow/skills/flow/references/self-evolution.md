...
The keystone gate survives exactly where the risk is: leaf fixes flow through unattended (the run self-merges, or the janitor mops up an orphan); failing CI and conflicts always wait for a human. Hot machinery waits for a human too — EXCEPT in this maintainer self-target repo, where a green + leaf hot PR may auto-merge (in-run via the `merge` stage, or via the reap janitor for an orphan) under `[evolve] auto_merge_hot` (default off, on solely here) + isolation (one hot at a time, fleet quiesced) + the in-run/reap guard-property diff review. For user projects the flag stays off and the human-merge keystone holds for hot. Veto any auto-merge by converting its PR to draft (or closing it) before the next janitor pass.
...
- **machinery_edit flock + atomic write.** A fleet runs many `/flow` jobs at once and several can hit reflect together; the raw Edit tool has no cross-process serialization, so two concurrent machinery edits to the same flow source race (lost update, or a torn read that crashes a third run importing the half-written module). `machinery_edit.py apply` holds a single global flock across the whole read → replace → atomic-write, so concurrent writers serialize and any concurrent reader sees old-or-new, never a torn file. The flock is an OS advisory lock released on process exit — no lease to clear, nothing to leak on a crash. It also enforces the snapshot caveat and protected-branch refusal below (refuses `stage-registry.toml`, out-of-tree paths, and a protected-branch skill-root). Do not route machinery fixes through the raw Edit tool.
- **Snapshot caveat.** Never self-edit `stage-registry.toml` or a WIRED handler skill mid-run — they are in the run's canonical snapshot, so editing them trips the drift guard and aborts the very run making the fix. Those go PROPOSE + RECORD, or apply then `/flow recover reload-snapshot`. The snapshot also hashes the MAIN checkout's engine tree while that checkout sits on a protected branch (the `engine` component, flow-2pp) — this forbids nothing new for self-edits (machinery_edit already refuses a protected-branch skill root, and worktree copies are not hashed), but it means a mid-run `git pull` / `claude plugin marketplace update` on the main checkout now aborts in-flight runs instead of silently swapping their engine.
...
- **Human-merge keystone.** Only green + leaf PRs auto-merge. Non-green / conflicted always wait for a human. Hot waits for a human too, EXCEPT the deliberately-narrow maintainer-only exception: in this self-target repo a green + leaf hot PR may auto-merge (in-run via `merge`, or via the reap janitor for an orphan) gated by `[evolve] auto_merge_hot` (default off) + isolation (one hot at a time, fleet quiesced) + a guard-property diff review. For user projects the flag stays off and the keystone holds for hot. Widen the envelope only with eyes open.
