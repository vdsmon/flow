...
- **Human-merge keystone.** Only green + leaf PRs auto-merge: non-green and conflicted always wait for a human, and a hot leaf PR waits too UNLESS this is the maintainer self-target with `[evolve] auto_merge_hot` on (gated by one-hot isolation + CI-green + a guard-property diff review). The auto-merge envelope is deliberately narrow — widen it only with eyes open.
