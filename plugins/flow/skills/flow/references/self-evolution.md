...
- **Human-merge keystone.** Only green + leaf PRs auto-merge. Non-green and conflicted PRs always wait for a human. Hot PRs also wait for a human by default; the sole exception is this maintainer self-target repo under all four conditions: `[evolve] auto_merge_hot` flag set, one hot in-flight at a time (isolation), CI-green, and an independent agent diff review (guard reviewer). That exception is off for user projects — the keystone holds there unconditionally. The auto-merge envelope is deliberately narrow — widen it only with eyes open.
