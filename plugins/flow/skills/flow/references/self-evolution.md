...
- **Human-merge keystone.** Only green + leaf PRs auto-merge. Non-green / conflicted always wait for a human. Hot PRs also wait for a human UNLESS `[evolve] auto_merge_hot` is on (maintainer self-target only, default off); when on, exactly one hot PR may auto-merge per pass after a guard-property review. The auto-merge envelope is deliberately narrow — widen it only with eyes open.
