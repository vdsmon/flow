...
1. `<ticket-dir>/dispositions.json`'s **fix pile** if the file exists — the `threads[]` entries with `"disposition": "fix"`, each carrying `file` / `line` / `title` / `body` (the same work-list shape as the review threads in source 3 below). This is the durable disposition set an interactive `revise` persists (`references/verb-revise.md` step 5a; the schema and the fix-pile definition live in `references/review-packet.md`). When the file exists it is AUTHORITATIVE — an explicit empty fix pile means "nothing to fix here", NOT a fall-through to source 3. Union it with `instruction.md` (source 2) when BOTH exist.
2. `<ticket-dir>/instruction.md` if it exists — a free-text change-request the maintainer gave to `/flow revise`. Its text IS the work to do; treat it as the plan.
...
4. Implement the production code.
   ...
   **Code comments.** Self-document first: try a rename or restructure that removes the need for the comment. Write one only when a non-obvious *why* survives, or for a workaround (name the cause), a surprising invariant, or an unavoidably dense expression (regex, bit-twiddle). Never restate the code, narrate obvious flow, or repeat a rationale that a nearby comment or docstring already carries (state it once at the definition and point to it elsewhere; the module docstring, a constant's comment, and its use site must not all tell the same story).
   ...
   ```bash
   ${CLAUDE_SKILL_DIR}/scripts/lint_comments.py --diff-base <started_at_sha> <touched-files>
   ```
...
5. Run the project's FULL CI-equivalent gate before declaring green — not just the tests.
   ...
   **Run every gate part in the FOREGROUND, chunked to stay short.** This stage runs as a spawned subagent (`subagent:general-purpose`), and a spawned subagent does NOT receive background-task completions after its turn ends — those notifications route to the top-level orchestrator session. So a `run_in_background` / `Monitor` command here ends your turn "waiting for the notification" and the stage stalls until the orchestrator `SendMessage`-resumes it (FT-1328, seen twice). NEVER `run_in_background` and NEVER `Monitor` a command in this subagent. This holds even in an ATTENDED run, so it is not the headless turn-boundary fallback of `references/stage-review_loop.md` §1 — it is unconditional. Run each gate part as a plain foreground `Bash` call with an explicit `timeout` <= 600000ms (the Bash ceiling), and chunk so no single call runs long: for this repo `mise run test` is already two invocations against distinct rootdirs (`pytest scripts/tests` then `pytest hooks/tests`), so run them as separate foreground calls; split further by path / module / marker if any one part is slow, and count green only when every chunk passed. Keeping each chunk short also dodges the ~360s harness idle-watchdog (flow-rbr), a DIFFERENT kill that fires below the 600000ms ceiling on a long quiet run. Do NOT wrap a test run in a `sleep`-poll loop: a pytest run is one long command, not an async-state wait. When this stage instead runs inline (handler `inline`, a backgrounded run), the same turn-boundary / idle risk hits the orchestrator's own gate run, so the same chunked-foreground-with-`timeout` rule holds (cf. `references/stage-review_loop.md` §1).
