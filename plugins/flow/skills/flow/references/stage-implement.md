...
2. Confine edits to the planned files.
   ...
   **Plugin version bump — mandatory when `planned_files` holds `plugins/flow/.claude-plugin/plugin.json`; recompute against the worktree, do NOT copy the plan's number.** Whenever the stamped `planned_files` contains `plugins/flow/.claude-plugin/plugin.json`, you MUST bump the plugin version — regardless of whether the plan text explicitly calls for it (the bootstrap auto-adds `plugin.json` + `marketplace.json` to `planned_files` for any `plugins/flow/` change, per the CLAUDE.md version-bump invariant, so their presence in `planned_files` IS the trigger, not the plan's prose). Any version number that appears in the plan is advisory only: the plan/spec stage computed it pre-bootstrap against the launcher checkout (`${CLAUDE_SKILL_DIR}`), which can lag the worktree base (`origin/main` for `--auto` runs), so the stated number may already be consumed by the published version. Read the current `version` from `plugins/flow/.claude-plugin/plugin.json` in the worktree, always increment the PATCH component, never the minor (e.g. `0.25.12` → `0.25.13`); change only the patch digit, do not reformat the surrounding JSON. Bump the `.claude-plugin/marketplace.json` flow entry to the identical value, in sync (the CLAUDE.md version-bump invariant). Concurrent evolve PRs branch off the same base (`flow_worktree.py create --base @default` fetches the same `origin/main`), so a deterministic patch bump makes every concurrent PR emit a byte-identical version hunk; git's 3-way merge then auto-resolves them instead of N-1 going DIRTY (flow-8we). Reading the worktree's own current version is correct for both `--auto` runs (based on `@default`) and interactive runs (based on the integration branch).
