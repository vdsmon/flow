...
## Execution model: routed implementer vs inline

When the run's route snapshot activates the `implementer` profile (`activation: pending`), this stage runs as a routed capsule writer: the implementer edits and tests inside a private capsule cloned at the sealed `source_sha`, and Flow, not the model, captures the binary-aware Git patch and imports it into the authoritative worktree under a sole-writer, compare-and-swap claim. The order's `allowed_mutation_paths` is sealed to the run's `planned_files` (from `baseline.json`, the same set the content-ownership commit gate re-scans), so the capsule may touch ONLY the planned files: a touch outside that set makes the whole change an `ownership_violation` and nothing is imported. The worker returns a typed report only (`summary`, `evidence`, `source_sha`); it never serializes a diff and never commits.

When the profile is shadow, legacy, or the stage runs inline, the host agent edits the authoritative worktree directly and the commit gate enforces the same planned-file boundary downstream. The Steps below are written for that host-edit path; the routed capsule writer follows the same TDD discipline and the same planned-file boundary, but its edits reach the worktree through the guarded import rather than landing in it directly.
...
2. Confine edits to the planned files.
   ...
   **The routed implementer (capsule writer) has no widening escape hatch.** Under the routed path, `allowed_mutation_paths` is sealed to `planned_files` when the order is prepared, and touching any path outside it makes the whole capsule patch an `ownership_violation` that imports nothing (there is no partial import and no chance to widen after the fact). If the routed implementer finds it needs a file outside the planned set, it must STOP and report that the planned set is incomplete rather than edit outside it; widening the set and re-running is the orchestrator's job. The add-and-flag path above applies only to the inline/host-edit run, where the post-implement reconcile widens `planned_files` and re-records the baseline before commit.
