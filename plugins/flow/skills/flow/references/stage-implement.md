...
2. Confine edits to the planned files.
   ...
   **Plugin version bump — recompute against the worktree, do NOT copy the plan's number.** If the plan calls for a plugin version bump, the number it states is advisory only: the plan/spec stage computed it pre-bootstrap against the launcher checkout (`${CLAUDE_SKILL_DIR}`), which can lag the worktree base (`origin/main` for `--auto` runs), so the stated number may already be consumed by the published version. Read the current `version` from `plugins/flow/.claude-plugin/plugin.json` in the worktree, increment it (patch for a bugfix / papercut, minor for new behavior), and bump the `.claude-plugin/marketplace.json` flow entry to the same value in sync (the CLAUDE.md version-bump invariant). Reading the worktree's own current version is correct for both `--auto` runs (based on `@default`) and interactive runs (based on the integration branch).
