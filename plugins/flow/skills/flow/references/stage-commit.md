...
2. Capture the implement-stage diff (idempotent if already captured):
   ...
   - Exit 1 with stderr `baseline.json planned_files is empty` → the run reached implement with an empty planned set, so there is no scope to capture and a repo-wide diff is refused. Do NOT retry implement: re-recording writes the same empty planned set and the capture fails identically, so the retry loops. Fix the inputs instead: put the files the run edited into the ticket frontmatter `planned_files` (or pass them to `record-baseline --files ...`), then rerun this step.
...
2b. Content-ownership gate. Verify the branch carries only planned changes before the commit is composed, because a PR must hold only what was planned. The scan covers the full delta against the recorded baseline: commits made since `baseline.origin_sha` AND the dirty working tree, so a change already committed on the branch is flagged the same as an uncommitted edit (committing a stray file does not hide it). `origin_sha` is the run's own origin, written once and preserved by every later record, so the post-implement reconcile widens `planned_files` without moving what this gate scans from. That widening means a legitimately-touched file is owned by now; anything still outside it is unplanned and must not ride along. The printed JSON says what was scanned: `ownership_anchor` and `anchor_source` (`head_sha_fallback` on a baseline written before `origin_sha` existed), plus `committed_scan_empty` for the committed half. Read `committed_scan_empty` narrowly. It is true of any run whose work is still uncommitted, which is the normal case and is checked in full by the working-tree half. Only `ok` together with an empty `changed` AND `committed_scan_empty` describes a scan that covered nothing.
   ```bash
