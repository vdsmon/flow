...
- **Real issue, in a flow workspace:** offer `FLOW ticket create --request "<the root cause and proposed fix>"`, and note the new key then runs as an ordinary `FLOW <key>` delivery, in this conversation or a fresh one. When the verdict is a live production incident, offer the hotfix form instead: `FLOW ticket create --hotfix --request "<the root cause and proposed fix>"`, whose created key runs as `FLOW <KEY> --hotfix` (the expedited lane, `references/command-target.md` §Target options); the human still chooses, and the ordinary form stays available for an incident that can wait. When the human instead says fix it inline here, the investigation conversation simply continues into the named target's ordinary lifecycle, exactly the FT-1569 shape (2026-08-04: investigate found the defect, the human said "do it inline here", and the same session delivered the PR).
