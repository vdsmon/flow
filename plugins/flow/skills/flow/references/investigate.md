...
The report ends with the delivery offer, and the human chooses; investigation never starts a run on its own.

- **In a flow workspace:** offer `FLOW ticket create --request "<the root cause and proposed fix>"` to capture the finding as a ticket, and note that the new key then runs as an ordinary `FLOW <key>` delivery, in this conversation or a fresh one. When the human instead says fix it inline here, the investigation conversation simply continues into the named target's ordinary lifecycle, exactly the FT-1569 shape (2026-08-04: investigate found the defect, the human said "do it inline here", and the same session delivered the PR).
- **Outside any flow workspace** (the command is workspace-optional): deliver the report and, when a fix belongs in a repo that IS an initialized flow workspace, say so and name the `FLOW ticket create` invocation to run there. No workspace means no tracker write is offered at all.
