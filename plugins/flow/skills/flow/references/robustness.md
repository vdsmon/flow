...
A change touching any of these files is a hot change: `triage.py`'s `_GUARD_FILES` is the authoritative set, `triage.is_hot_change` the decider, and the two prose enumerations (verb-evolve audit step 2, stage-reflect step 2b) are seam-checked equal to it. Hot rides the full lane and the guard-property review regardless of labels — see `verb-evolve.md`.
