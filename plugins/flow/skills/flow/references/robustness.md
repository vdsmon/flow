...
| Content-ownership commit gate | A run committing changes outside its planned file set, including a change smuggled in via a rogue `git commit` mid-implement. | `diff_extract.py` `check-ownership`: refuses when the branch delta (dirty tree AND commits since `baseline.origin_sha`) leaves `planned_files`. `origin_sha` is written on the first record and preserved by every later one, so a re-record widens the owned set without shrinking the range scanned. The result also names its anchor and whether the committed half found anything, so a green answer states what it covered. This is the mechanism AGENTS.md means by "content-ownership commit gate". | Went branch-wide after committed out-of-baseline changes slipped past the dirty-tree-only check (#393). Refuses an out-of-scope staged rename by collecting the unowned source side (#366). Moved to `origin_sha` after the gate was measured passing an unplanned committed file: the post-implement reconcile re-recorded `head_sha` to live HEAD, so the committed range was empty and checked nothing (flow-c6gq). |
...
A change touching any of these files is a hot change: `triage.py`'s `_GUARD_FILES` is the authoritative set, `triage.is_hot_change` the decider, and stage-reflect.md's enumeration of it is generated by `module_map.py` (checked via `seam_check.py`), so the prose cannot drift. Hot rides the full lane (`tier_policy.lane_for`: hot always resolves full) and the merge-time guard-property review (`scrutinize.md` §Merging) regardless of labels.
