...
2. The middle loop is `FLOW maintain backlog drain` or
   `FLOW maintain evolution drain`: a driver session repeatedly classifies durable
   evidence and runs a bounded worker fleet for already-authorized work.
3. The outer loop is an optional scheduler that invokes producer and consumer driver
...
receipt, durable decision, or evidence-specific block. A drain ends when existing
work is settled or blocked, or when fresh candidates are returned for attended
planning. A scheduled fire records explicit
start/end/outcome evidence. No loop infers completion from a vanished process handle.

## Driver and native agents

The active Claude Code or Codex conversation is the driver. Native collaboration
agents are bounded by `min(configured, host_capacity - 1)`, leaving one slot for the
driver. The driver registers durable fleet intent before any authorized delivery
agent starts, waits through the host adapter, and cancels its own handles on ordinary
exit. After driver loss, a new driver
...
recurrent evidence into bounded work. Attended ticket runs deliver accepted work. Immutable ship
...
The optional scheduler templates in `ops/` invoke synchronous driver sessions for
