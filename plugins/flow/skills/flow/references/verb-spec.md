...
4. **Decided-mode probe — then the headless plan.**
   ...
   Then get the same INDEPENDENT confidence rating as interactive step 4 — call `advisor()` (or a `general-purpose` `Agent` if advisor is absent; on a Fable model skip the probe, advisor is absent by design) over the captured plan. Its score feeds the branch below.
