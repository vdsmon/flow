...
4. Iterate the implementation plan with the user: goal, files to change, approach, test strategy, risks.
   ...
   **Propose a verification lane (part of the plan).** Scale the run's verification to the cost of being wrong (the xqt verdict — see `references/verb-evolve.md`). Assess this ticket the way the evolve producer audits a finding (§audit step 2) and propose one lane in the plan:
   - **`express`** — mechanical, tightly-bounded, behavior-preserving (a doc/comment fix, a format-preserving rename, a proven-dead-code deletion). Skips the confidence probe (below), relaxes the implement stage's mandatory-new-test, and collapses reflect to friction-log-only.
   - **`light`** — small-footprint (at most 2 planned files, no touched file over ~500 lines, no guard/cross-file seam) AND behavior-preserving OR behavior-changing with NO checkable spec invariant. Skips the confidence probe; keeps full TDD.
   - **`full`** (the default) — everything else: any behavior change carrying a checkable spec invariant (a sign / unit / rounding / ordering / count / stated bound), broad blast radius, or anything you are not sure is bounded.

   Be **conservative**: propose `express`/`light` only when the work clearly fits; when unsure, propose `full`. **Hot-clamp first — compute the EFFECTIVE lane.** A **hot change** (a planned file in the guard set — `lease.py` / `snapshot.py` / `state.py` / `dispatch_stage.py` / `diff_extract.py` / `flow_worktree.py` / the other safety-machinery files, `SKILL.md`, `stage-registry.toml`, `CLAUDE.md`, a wired handler) forces the effective lane to `full` REGARDLESS of what was proposed or passed via `--lane`/natural language. Apply this clamp yourself, here, before the confidence decision below — do not treat `create`'s re-clamp as the only line of defense, or you would skip the probe on a lane the run will not actually take. `triage.is_hot_change(<planned_files>)` is the predicate (`triage.py decided --files "<planned_files>"` exposes `is_hot` if you want to confirm). Surface the EFFECTIVE lane as a one-line **`## Lane`** section in the plan with a one-clause rationale (e.g. "express — comment-only fix" or "full — touches `state.py` (guard), hot-clamped"), so approving the plan approves the lane. **If the user passed `--lane`** (or stated a lane in natural language — "this is trivial, express it"), use that as the proposed lane, then apply the hot-clamp; still show the result at the gate. The user can override at the gate ("use full instead" / "trivial, express it").

   **Confidence rating (MUST when the lane is `full`, before step 5's gate) — assessed independently, not self-scored.** A plan's author is the worst judge of its confidence; optimism bias makes a self-reported score self-justifying. Hand it to a second mind. Preferred: the `advisor` tool — it auto-forwards the full transcript (the ticket, your exploration, the drafted plan), so in the SAME turn state what you want back — **Score (0-100%)**, **Proven** (bullets directly verified: code read, spec quoted, real data/DB inspected), **Inferred** (from convention / naming / a 1:1-chain argument), **What would raise it** (concrete reachable artefacts) — then call `advisor()`. **Fable short-circuit:** when the session model is Fable (`claude-fable-*`), `advisor` is absent by design and not needed — skip the `ToolSearch` probe and go straight to the fallback. For other models, fall back only after a `ToolSearch` for `advisor` returns nothing, or if `advisor` errors / is rate-limited. The fallback: spawn a `general-purpose` `Agent` instead, handing it the ticket context + the drafted plan text + that same rubric. Record the result as the plan's `## Confidence` section, attributed to the assessor. Library-API claims (a Polars/Pandas idiom, a framework hook, an SDK call) must be Context7-verified, never left under "Inferred".
   **When the EFFECTIVE lane (after the hot-clamp above) is `express` or `light`, SKIP the rating** — the user's approval at the spec gate is the vetting (the same role the Opus producer's audit plays for a drain-stamped tier label), so the independent human gate replaces the independent advisor probe. Key off the effective lane, NOT the raw `--lane`/proposal: a hot change clamped to `full` runs the full probe even if `express` was requested (that is precisely the change you most want a confidence probe on). Record `## Confidence` as `lane: <express|light> — confidence probe skipped per lane`. Library-API claims still get Context7-verified where the plan makes one. (A lane downshift never removes the human gate, only the redundant advisor call.)
...
6. (Normal mode) Persist the approved plan and bootstrap the worktree.
   ...
   python3 ${CLAUDE_SKILL_DIR}/scripts/flow_worktree.py create \
     ...
     --lane "<the approved lane: express|light — OMIT for full>"
   ```
   ...
   **`--lane`** carries the lane the user approved at the gate — pass `express` or `light`, OMIT it for `full` (the default an absent field means; `create` leaves the frontmatter `lane` unstamped). `create` clamps a hot change (a guard file in `--planned-files`) back to `full` regardless of what is passed. Omitting `--lane` entirely falls back to the bead's tier labels (the drain path) — for an interactive run with no tier label that resolves to `full`, so the explicit flag is how an interactive run reaches `express`/`light`.
   The bootstrap seeds state (plan pre-completed, ticket left pending), injects the plan, stamps `planned_files` + `commit_type` + `commit_summary` (+ `e2e_recipe` / `covers` / `lane` when given) into frontmatter (so the implement pre-hook, the commit stage, and the e2e stage never pause to ask the user — which is what lets the tail run unattended if you background it), points the worktree's memory store at this checkout's `.flow` (shared, so memory compounds across worktrees), copies gitignored config, and `mise trust`s the worktree.
