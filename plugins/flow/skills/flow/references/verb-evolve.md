...
- **robustness** — real gaps in the load-bearing machinery: the four correctness guards (run lease, snapshot TOCTOU, atomic writes + quarantine, content-ownership commit gate) on the flock substrate, plus friction logging as the evolution feedstock; threat→file map in `references/robustness.md`. Tighten, never erode.
...
**Parked-by-design PRs (a job-scan misread as deaths).** A session-gone + PR-open+green scan result can be a parked-by-design run rather than an orphan this reap step should merge. When a completed run's bead is non-`evolve` (unlabeled, or `proposal`), it took the merge stage's gate skip: `evolve_self_merge.decide()` returned `{action: "skip", reason: "not an evolve bead"}`, the run finished `completed`, and its `--auto` self-teardown (`references/verb-do.md` §Self-teardown) removed the session/job dir ~30s later. The PR stays open+green with no live session, so a job scan can read it as a mid-run death even though the run parked its own PR by design for a maintainer (or `--include-proposals`) merge. Four terminal shapes to tell apart: **self-merged in-run** (an `evolve` leaf took the Layer-2 path and closed its own PR, so this reap step never sees it), **gate-skip parked** (the population here, a completed run awaiting a human or `--include-proposals` merge), **pre-PR stranded** (died before opening a PR, the §Recover channel below owns it), and **genuine post-PR orphan** (opened a PR, then died before self-merging, the case this reap step exists to absorb). One check separates a parked run from a death: the un-reaped worktree's `.flow/runs/<key>/state.json` reads `stages.merge == completed` and `stages/merge.out` carries the probe verdict (`action: "skip"`, whichever reason: a held hot bead or an eval-driven skip parks the same way); that worktree is parked by design for the reap to leave for the human, and re-filing it as an orphan death is the false positive to avoid. flow-wbqq incident (2026-07-09/10 special drain): of the 9 runs a job scan filed as orphaned, 5 were gate-skips on unlabeled/`proposal` beads and 3 had actually self-merged in-run (the scan conflated dead first-wave attempts with the relaunched sessions that finished the work); the only genuine deaths were pre-PR launch-wave sessions the §Recover channel already handles.
...
**Permission-classifier denial (unattended runs).** In an unattended session the Claude Code auto-mode permission classifier may DENY the `gh pr merge` ("Merge Without Review … does not specifically authorize merging this PR"). That is a harness permission boundary working as designed, not a flow gate failure — the green PR is a complete deliverable. Park the PR and report it; the remedy for genuinely unattended self-merge is per-PR approval when attended, or a specific Bash allow-rule for the merge command (see `troubleshooting.md` §gh).
...
2. **`open-question`, OR no `[defer-reason: ...]` tag at all, OR the bead carries the `hitl` label** (a pre-marker defer, a defer carrying a substantive question, or a decision-bound bead) → LEAVE deferred — the normal `/flow triage` path where a human answers. Skip; do NOT reopen. A `hitl` bead is left deferred REGARDLESS of its `[defer-reason:]` tag: it resolves only through a live exchange, so no model-escalation ladder applies; clearing it is the maintainer's `/flow triage` reopen (which removes the label).
...
- **`wait`** (launch empty, but a **blocking** in-flight run remains) → go to **D-wait**. A run blocks when its lease reads `live` OR `corrupt`: a live run will self-merge and free serialization/backpressure; a corrupt lease (run.lock unparseable, ownership unconfirmable) does NOT self-free — it blocks until a human runs `recover takeover`. A non-empty `launched_pending` is a third blocking reason: a launched-but-pre-lease run (in the fleet ledger, no branch/lease/PR registered yet) has no run dir to read, so it would otherwise look non-blocking — it blocks until it registers (then the lease/PR channels take over) or its fleet entry ages past `STALE_AFTER_S` (1800s). It is NOT parked. All route to **D-wait**.
