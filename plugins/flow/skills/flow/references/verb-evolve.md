...
- **doc drift** — two sub-classes. (a) *claims vs the actual code* — `MODULE.md` / `inventory.md` / `SKILL.md` / `references/*.md` asserting behavior the code no longer has. (b) *cross-file vocabulary drift* — a PR that introduces or renames a term, leaving the retired phrasing in sibling reference docs describing the SAME subsystem. When a finding is a renamed/retired term, do NOT scope it to the one file the diff surfaced: `grep` the WHOLE `references/*.md` + `SKILL.md` doc set for the stale phrasing and enumerate EVERY matching `file:line` site in ONE bead, because a single PR's prose drift typically hits every reference file describing that subsystem. (flow-t97: the drain-termination vocab drifted in BOTH `verb-evolve.md` and `self-evolution.md`; scoping the bead to one file forced a mid-run RECONCILE when the downstream run discovered the second locus.) `flow_beads_create.py` (§3) has no per-file lever — the relevant content levers are `--description` and `--dedup-key` — so the loci live in the `--description`: file ONE finding whose description cites every locus (all `file:line` sites), so the downstream run's plan stage enumerates them into its "Files to change" and the bootstrap stamps a complete `planned_files`. The audit step itself does not stamp `planned_files`; never split a single drift into per-file beads.
