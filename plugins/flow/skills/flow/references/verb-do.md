...
When an inline stage's prose (a handler running in the orchestrator's own main conversation) invokes a Skill, the skill's instructions arrive as a prompt and the orchestrator's reply IS the skill's output — which by default ends the turn. Ending the turn there strands the rest of the stage (its remaining file writes, its script call, its `.out` capture) and whatever real boundary should follow, so a do-loop stage stalls until the user pokes. The rule: never let the skill's output be the turn boundary — carry it straight into the stage's remaining steps in the SAME reply, on to the stage's next legitimate boundary. For a do-loop stage that boundary is the do-loop `advance`, so run right through to it; for a verb with its own human confirm gate (e.g. the `new` verb's Step 4 preview), continue up to that gate, which is a legitimate stop rather than the skill-output turn boundary the rule forbids.

This governs INLINE handlers only. A `subagent:<type>` handler's skill runs inside the subagent loop, and its output rides back inside the subagent's report rather than landing as a turn the orchestrator is holding, so it is OUT of this rule — the orchestrator advances that stage from the returned report as usual. (This is why stage-implement.md's humanize code-comment pass, which runs inside the implement subagent, is not covered here.)
