...
3. When the workspace configures `[preflight] credential_probe` (workspace.toml), the dispatcher has already run the silent probe before emitting this stage's descriptor and refused dispatch on failure (the stage stays pending until the credential is refreshed), so a descriptor in hand means the credential answered moments ago (the descriptor's `preflight` field carries that probe's result record as the durable proof). Re-run the probe yourself only when meaningful time passed since dispatch, and before any long or destructive recipe step:
