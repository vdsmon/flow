---
name: e2e-runner
description: Flow's dispatched handler for the `e2e` stage. Flow's dispatcher spawns this agent type directly from the `subagent:flow:e2e-runner` handler in `[pipeline.handlers]`; it is not meant for direct invocation outside a Flow run.
---
