...
The floor is untouched by all of this, once and plainly: guards are `hot`, ride the hot path, and no safety property is ever dropped regardless of which model runs — speed is authorized only where a mistake is reversible and bounded. This section is itself the deliberate maintainer decision that authorizes the cheaper tier for producer-vetted-trivial work; the loop may act on it without re-asking per instance, and it widens no gate.
