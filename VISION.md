...
The concrete case that prompted this: a capable smaller model (Sonnet) handling work an Opus producer already stamped trivial is a win to take, not a risk to gate. The producer read the code and judged the work mechanical and non-guard; trusting that judgment downstream is the leverage, and a capable cheaper model clearing what a stronger one vetted is a safe bet, not a gamble. The trivial set is non-`hot` by construction, so it structurally never touches the safety machinery — the blast radius of "it broke" is fenced to exactly the class where breaking is cheap.

This does not touch the floor. The section above stands unchanged: guards are `hot`, ride the hot path, and no safety property is ever dropped regardless of which model runs. "Move fast" lives on the product surface, never the safety properties — speed is authorized where a mistake is reversible and bounded, and nowhere it is not. Two conditions keep it honest rather than reckless. **Visible:** a downshifted (or otherwise speed-favoured) run must be recorded, so a bad PR is instantly correlated to its tier, reverted, and the pattern un-stamped — move-fast-and-break-things works only when the breakage is seen, not when it is silent. For the behavior-changing slice of that set, *recorded* means more than the tier label: the run's acceptance invariant — the DoD assertion it was meant to satisfy (a sign, bound, ordering) — is recorded as a correlatable field on the ship event, so its expected behavior is a checkable assertion rather than an assumption that green CI implies correct. (Recorded and correlatable, not enforced: the producer still states the invariant, and an omission shows up as an empty field — strictly more than a prose convention, not a guarantee of presence. The reversible-and-bounded trivial floor is untouched; only the behavior-changing class gains the recorded assertion.) **Measurable:** the win must show up under `recall --metric` (the same bar the rest of this document holds); a cheaper or faster path that does not register as real savings is vibes, and gets reverted like any other change that didn't pay for itself.

One scope note for the loop, since the floor section is strict about gates: this section *is* the deliberate maintainer decision that authorizes choosing a cheaper worker tier for producer-vetted-trivial work — the loop may act on it without re-asking per instance. It widens no safety gate; the rule above (a guard gate widens only by a deliberate maintainer decision, never inferred) is untouched.
