...
Between us sits the foreman — the self-target maintenance seat, the long-lived session that owns what gets worked and when: triage, grouping, sequencing, amendments, and merges on met gates. Its authority is delegated and auditable; my instruction overrules it, and plan approval never leaves me.
...
It is the scoring anchor for the foreman and reflect: serves the thesis, on the right side of the auto-vs-propose line, doesn't erode the floor → candidate; adds surface or can't be anchored here → drop or escalate. Propose the bold thing; let me triage it. This document governs flow improving itself, and it applies only to flow's own repo. Flow runs in two kinds of workspaces, both mine: this repo — the self-target, where the foreman's merge authority and the machinery beads live — and my delivery workspaces, the work repos and side projects where the human-merge keystone holds and this vision does not reach. Where the docs say "delivery workspace," read: me, wearing the user hat in a workspace that is not this repo. Same person, different role; the guardrails between the two roles are load-bearing, not multi-user residue.
