...
One human, many operators. I am the only human flow will ever have — no second user is coming, and that is a settled fact, not modesty. But flow is not a single-user system: its working population is agent sessions — every driver, worker, reviewer, and janitor that operates it. They are who the rigor is for. The enforced-true docs, the closed vocabularies, the generated surfaces exist so a fresh agent session can orient in minutes and be trusted with autonomy. None of it was ever onboarding for another human, and none of it may be cut on the grounds that no human needs it.
...
- **Not adoption-driven.** Success is my friction going down, never stars or installs. The repo stays public because the ideas are worth reading — a reference others may mine, never a product anyone installs; nothing in it owes a stranger an onboarding.
- **Not portable.** macOS is the only interactive environment flow will ever run on. Windows support is permanently out of scope; the Linux CI runner is an incidental executor of a stdlib-only suite, not a supported target.
- **Not future-proofed.** No option axis for a workflow I don't run, no accommodation for a hypothetical need — I build what I need the day I hit the wall. The counterweight is change amplification, not flexibility: seams stay cheap to change so the system bends the day I need it to; unused options die.
...
*Software can improve and heal itself when a capable agent is given a real loop to do it through.* What is demonstrated so far is narrow and real: a guarded self-repair loop that, under human-gated conditions, finds blockers in its own machinery and fixes them mid-run. That is the mechanism working, not the broad thesis settled. The open question is no longer whether a harness can evolve itself at all; it is how far, and under what guardrails — and this document is the guardrail on the "how far."
...
**Propose boldly. Ship without me only on ground truth or a met gate.**

No fear on the proposing side: the bigger the improvement to the system, the more it is wanted. A monumental change that challenges the whole architecture is as valid as a one-line fix when it makes us faster and more reliable. The gate is never on imagination — it is only on what ships without my eyes on it.
...
Between us sits the scrutiny — the post-hoc seat, invoked between runs, that sweeps every workspace's run evidence, triages the queue with veto power, mints the machinery backlog, and merges this repo's parked PRs on met gates. Its authority is delegated and auditable; my instruction overrules it, and plan approval never leaves me.
...
Autonomy is earned by guards, not assumed. The safety machinery is what makes delegating flow's operation safe at all: the four correctness guards — the run lease (one run per ticket), the canonical-snapshot drift guard (the harness config can't change mid-run undetected), atomic writes and quarantine (no step reads half-written state), and the content-ownership commit gate (a PR holds only what was planned) — on the flock substrate that serializes self-edits, plus friction logging as the evolution feedstock.
...
Green does not enforce that line — green proves the tests pass, not that the guard still protects you, and most of these properties have no direct test. So the seat's merge checklist carries it: before a guard change merges, the review must answer "does this remove a protection?" and block if it does. CI and the seam-checker are observations to verify, not guarantees — a skipped workflow, a misread rollup, or a stale local plugin can make green lie. Simplify presentation, never the safety machinery. This document does not pre-authorize widening any gate; that is my decision, made deliberately, like the ones that produced this section, never inferred by the loop.

## Move fast where breakage is cheap

A deliberate counterweight to the floor above. flow is a skill, not mission-critical software: when a trivial change lands slightly wrong, the cost is a `git revert`, not an outage. That reversibility is an asset I want the loop to spend, not hoard. On the reversible execution surface — which model does the work, how a vetted-trivial task is tackled — the posture is move fast: take the cheap win, and treat the rare bad merge as the cheap, visible, recoverable event it is, not a risk to gate against in advance.

The concrete case that prompted this: a capable smaller model handling work a stronger producer already stamped trivial is a win to take, not a risk to gate — the model and effort tiers ship as opinionated defaults in the registry, the workspace's `[models]` hints override them, and the tier rides the run record. The producer read the code and judged the work mechanical and non-guard; trusting that judgment downstream is the leverage. The trivial set is non-`hot` by construction, so it structurally never touches the safety machinery — the blast radius of "it broke" is fenced to exactly the class where breaking is cheap.

Two conditions keep it honest rather than reckless. **Visible:** a downshifted (or otherwise speed-favoured) run must be recorded, so a bad PR is instantly correlated to its tier, reverted, and the pattern un-stamped — move-fast-and-break-things works only when the breakage is seen, not when it is silent. For the behavior-changing slice of that set, *recorded* means more than the tier label: the run's acceptance invariant — the DoD assertion it was meant to satisfy (a sign, bound, ordering) — is recorded as a correlatable field on the ship event, so its expected behavior is a checkable assertion rather than an assumption that green CI implies correct. **Measurable:** the win must show up under `FLOW measure` (the same bar the rest of this document holds); a cheaper or faster path that does not register as real savings is vibes, and gets reverted like any other change that didn't pay for itself.

The floor is untouched by all of this, once and plainly: guards are `hot`, ride the hot path, and no safety property is ever dropped regardless of which model runs — speed is authorized only where a mistake is reversible and bounded. This section is itself my deliberate decision authorizing the cheaper tier for producer-vetted-trivial work; the loop may act on it without re-asking per instance, and it widens no gate.

## Using this document

It is the scoring anchor for the seat and reflect: serves the thesis, on the right side of the auto-vs-propose line, doesn't erode the floor → candidate; adds surface or can't be anchored here → drop or escalate. Propose the bold thing; let me triage it. This document governs flow improving itself, and it applies only to flow's own repo. Flow runs in two kinds of workspaces, both mine: this repo — the self-target, where the seat's merge authority and the machinery beads live — and my delivery workspaces, the work repos and side projects where the human-merge keystone holds and this vision does not reach. Where the docs say "delivery workspace," read: me, wearing the user hat in a workspace that is not this repo. Same person, different role; the guardrails between the two roles are load-bearing, not multi-user residue.
