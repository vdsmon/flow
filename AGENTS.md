...
- **Merges are gate-checked, and hot merges carry the property review.** A parked green PR is merged by the manager (or the human) only when CI is green and review is clean; a HOT diff additionally passes the guard-property review (`references/manager.md` §Merging) — no safety property is ever dropped on green alone. In delivery workspaces the human-merge keystone holds.
