...
- **Branch off `origin/main`, never local `main` (lags) or current HEAD.** This repo churns with many worktrees; cutting a feature branch off a stale/feature HEAD pollutes the PR with already-merged commits (→ DIRTY). Flow runs resolve the remote default branch before creating their worktree; do the same by hand.
- **Live-testing plugin changes:** the `vdsmon-flow` marketplace tracks **`origin`** (`https://github.com/vdsmon/flow.git`) — a deliberate multi-machine setup with no local paths in the config. To exercise merged changes: wait for the post-merge version stamp on `main`, then `claude plugin marketplace update vdsmon-flow` (`claude plugin details flow` shows the version). Fast-forwarding the local checkout is workspace freshness (drivers mint worktrees off it), independent of plugin delivery.
...
- **Editing a `references/*.md` paragraph?** A paragraph or bullet there is often ONE physical line of 1000+ characters, and `lint-comments` scopes its em-dash check by changed line, so touching any part of such a line un-grandfathers every em-dash already on it, including text you did not write. Budget for rewording those, and keep the line unwrapped: hand-wrapping to shrink the diff violates the never-hand-wrap rule.
- **Adopting a plugin-provided agent type** (`subagent:flow:<name>` in `[pipeline.handlers]`) needs a session started AFTER the plugin update that ships it. The agent registry is compiled at session start, so a marketplace update does not reach a session already running, and a driver spawned from that session inherits its registry and fails to resolve the type. Update the plugin, start a fresh session, then adopt.
- **Mutation-checking a test?** Clear `__pycache__` or set `PYTHONDONTWRITEBYTECODE=1` first. A mutation that preserves the file's byte length (flipping `1` to `0`) is masked by the stale `.pyc`, because CPython invalidates on mtime AND size, so the test stays green and you wrongly conclude nothing guards the behavior. `pytest -B` is not the fix: `-B` is an interpreter flag and pytest rejects it.
...
- **Public grammar is generated.** `public-commands.toml` is the authored source; `public_commands_check.py` reports what is stale and `python3 public_commands_check.py write` regenerates SKILL.md's derived surfaces in place. A command's `usage` string and its `options` table are authored independently and nothing cross-validates them: only `usage` feeds the grammar block, so an options-only edit changes no generated surface, and a usage-only edit advertises a flag the router rejects at use time — edit both. Removed public forms must fail normally; never add aliases or migration redirects. Likewise MODULE.md's §Derived surfaces (import graph + subcommand names) is generated: after adding, removing, or re-importing a script, run `python3 module_map.py write`; `seam_check.py` fails while it is stale.
...
- **Merges are gate-checked, and hot merges carry the property review.** A parked green PR is merged by the scrutinize seat (or the human) only when CI is green and review is clean; a HOT diff additionally passes the guard-property review (`references/scrutinize.md` §Merging); no safety property is ever dropped on green alone. In delivery workspaces the human-merge keystone holds.
...
- **Fail-fast hooks are CHECK-ONLY.** The prek hooks never mutate files. Flow runs commit through the engine inside worktrees that share the main checkout's `.git`, so any installed hook fires during those commits too; a mutating hook (`ruff --fix`, a formatter writing) would create unowned drift against the content-ownership commit gate. The stage split lives in `.pre-commit-config.yaml`, which shells out to the repository checks so no rule set is redeclared against CI.
...
- **Dry-run gates every side effect.** `--dry-run` on any Flow command means no tracker write, no merge, no worktree reaping, no worker launch — with no exception for a write that happens to live inside a read-side function (a `bd create` reached through a read path is still a tracker write). A read that exists only to guard a write (a dedup scan) is part of the write and is gated with it; the dry-run path reports a `would_*` action record instead. `finalize.py` is the reference shape. (PR #514 carved the exception, PR #517 reversed it the same day; this line is the rule that would have settled it at review.)
