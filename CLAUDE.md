...
- **Version bumps.** `plugins/flow/.claude-plugin/plugin.json` and the `.claude-plugin/marketplace.json` flow entry stay in sync. The sync happens at merge time via the `version.py` stamp (stage-merge §3), not via a per-PR inline bump.
